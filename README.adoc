...
`git store cat <tree-ish>` decodes a bound document from the tree it names,
content-addressed and needing no name. `git store get <kind> <name>` resolves a
name first, and reports deleted entities differently from missing ones;
...
history. `Kind::read_as` is an explicit opt-in taking a `TargetSchema` the
caller selects, which `KindSchema::current_target` builds from the selected
`Kind`'s current published schema and history; when the source is absent from
that history or a migration edge is missing, the operation fails rather than
silently guessing or rewriting stored objects.

== Composable CLI Plumbing

The `git store` binary exposes small Git-shaped operations for manually scripted
migrations. It deliberately has no hard-coded `migrate` workflow. What it does
supply is the plumbing such a script needs: `git store ls <kind> --stale` is the
worklist of entities not bound to the kind's current schema tree, `git store get`
and `git store entity resolve` return the compare-and-swap token alongside the
value, and `git store document publish --batch` lands a rewritten set as one
...
`git store schema show <kind> --at <commit>` returns the schema snapshot at
that historical publication commit, reporting the kind, publication commit OID,
schema-tree OID, and human-readable field layout. Without `--at` it reads the
current snapshot; without `<kind>` it shows every published kind.
