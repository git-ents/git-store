...
*https://facet.rs[facet.rs] format crates for Git tree objects.*
...
| `value`
| no
| Enables the https://crates.io/crates/facet-value[`facet-value`] integration: exact 128-bit number text for dynamic writes, and the schema-driven reader (`deserialize_value_with_schema`, `validate_with_schema`). Dynamic serialization and the heuristic dynamic read work without it.
|===
...
Please refer to the in-source link:../../CONTRIBUTING.adoc[contribution guide].
