...
* *No format the reader must know.* The structure _is_ the Git tree. JSON
  ...
  is stored is the structural tree link:../facet-git-tree[`facet-git-tree`]
  produces.
...
link:examples/schemas.rs[`schemas`] example does the latter — the struct authors
the schema:
...
The checked-in link:schemas[`schemas/`] files were generated exactly this way.
