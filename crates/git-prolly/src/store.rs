...
impl<'repo> ProllyStore<'repo> {
    pub(crate) fn write_value(&self, value: &facet_value::Value) -> Result<ObjectId, Error> {
        let wire = crate::value::encode(value).map_err(|error| Error::Value(error.to_owned()))?;
        facet_git_tree::serialize_into(&wire, self.repo()).map_err(Error::Serialize)
    }

    pub(crate) fn read_value(&self, oid: ObjectId) -> Result<facet_value::Value, Error> {
        let wire: crate::value::WireValue =
            facet_git_tree::deserialize(&oid, self.repo()).map_err(Error::Deserialize)?;
        Ok(crate::value::decode(wire))
    }
