...
. A data document's root contains exactly `schema/` and `value/`.
. `schema/` contains the exact schema tree used to validate `value/`; ordinary
   reads decode only those two subtrees from the document commit.
. The schema document contains its kind name. A ref namespace is an index or
   publication mechanism, not the type definition.
. Newly written commits contain no `Schema:`, `Schema-Version:`, or `Ents-Ref:`
...
. The meta-schema is the reification of `facet::Shape`, has an empty-tree
   `schema` entry (`4b825dc...` for SHA-1 Git), and passes a fixed-point
   decode/re-encode check.
. A `doctor` check can verify the fixed point against a compile-time digest.
. Entity identity is derived from the stored content according to one
   documented rule and is used consistently; caller-selected names are not the
   authoritative identity.
. Deletion has an explicit, documented representation that remains
   distinguishable from an absent/pruned ref.
. Schema history and migration are explicit authoring/history operations, not
...
* `crates/git-store/src/main.rs`: CLI commands, including the future
  `doctor` command.
* `README.adoc` and `crates/gix-store/README.adoc`: currently document named refs
...
. constructs the canonical meta-schema;
. encodes it using the empty-tree schema identity;
. decodes it through the normal schema reader;
. re-encodes it;
. compares the resulting tree/object identity (and, where applicable, bytes)
...
. Define a schema identity type based on the schema tree/content digest.
. Make schema publication refs point to that identity/history, while storing
   the kind name inside the schema document.
. Add APIs that can decode a document from its embedded schema without a kind
   handle or schema ref.
. Retain named convenience APIs only as an optional index/compatibility layer,
   and make their delegation to content-based lookup explicit.
. Update migration APIs so they require an explicitly selected target schema
...
* update `README.adoc`, `crates/gix-store/README.adoc`, and relevant API docs to
...
. Phase 0 characterization tests and written contract.
. Phase 1 schema self-hosting/bootstrap and `doctor`.
. Phase 2 trailer removal.
. Phase 3 content-based schema/type lookup.
. Phase 4 entity identity migration.
. Phase 5 tombstones.
. Phase 6 compatibility and documentation cleanup.
