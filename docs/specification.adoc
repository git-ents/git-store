...
A reader MUST carry the recognized generations as compiled-in constants; the genesis tree of this format version is `09961be517a580c73b94d6b84b5a3e7dac2f0ac3`.
...
The schema-schema pin covers only which generation of `Node`/`Schema` a document's schema tree belongs to; it carries no information about the codec that interprets values against that schema (link:#serialization.schema-directed[schema-directed serialization], link:#deserialization.schema-driven[schema-driven deserialization]).
A codec change that leaves `Schema`'s own encoded shape untouched is invisible to this pin — the format currently has no mechanism that would catch it.
--
