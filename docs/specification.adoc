...
| `Struct`
| an `Object` with one member per field; every field the schema names MUST have a tree entry, `Optional` included, otherwise the read MUST fail.
...
| `Struct`
| an `Object` whose keys are all defined fields; every field the schema names MUST be present in the object, and a key the schema does not define MUST be an error.
