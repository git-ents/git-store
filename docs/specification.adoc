...
A `Node::Struct` field is a `StructField`, not a bare `Node`: `has_default` marks a field a write MAY leave without a tree entry, per link:#serialization.schema-directed[schema-directed serialization]'s `Struct` row, and a read finds such an omitted entry simply absent rather than failing, per link:#deserialization.schema-driven[schema-driven deserialization]'s. A struct enum variant's fields (`VariantKind::Struct`) carry no such marker and stay bare `Node`s.
...
A schema is published by serializing its `Schema` through this crate and pointing the ref at the resulting tree, or at a commit wrapping that tree for history and signing — the higher layer's choice.
====
...
A migration-schema generation's tree MUST carry a `codec` entry under exactly the same construction the schema-schema tower's does (link:#schema.representation.pin[the schema-schema pin]) — the identical fixture, spliced under the identical name, so the two towers share one content-addressed `codec` object regardless of which one writes it first.
...
[role="requirement", id="plumbing.schema.explicit"]
...
[role="requirement", id="plumbing.schema.history"]
.Historical Schema Addressing
--
`git store schema get <kind> --at <commit>` and
`git store schema inspect <kind> --at <commit>` MUST address the named schema
publication commit directly. Omitting `--at` from `schema get` selects the
current publication. Historical inspection MUST report the publication commit
OID and the schema-tree OID; it MUST NOT replace the historical selection with
the current schema.
--

[role="requirement", id="plumbing.inspect"]
...
[role="requirement", id="plumbing.output"]
...
[role="requirement", id="plumbing.cas"]
.Explicit Compare-and-Swap Publication
--
`git store document publish <kind> <document-tree> --expected <absent|OID>` MUST
perform a one-shot compare-and-swap. Without an alias, the expectation applies
to the canonical entity ref; with `--alias <name>`, it applies to that alias.
The canonical ref, optional alias, and materialized per-kind index MUST advance
in one ref-store batch or none of them may advance.

A stale expectation MUST fail with the compare-and-swap exit category and MUST
NOT be retried by the CLI. Objects written before a lost publication MAY remain
unreachable. Traversal, batching, retry, resume, and conflict policy belong to
the calling script or program.
--

[role="requirement", id="plumbing.identity"]
...
therefore contribute to identity. The canonical ref MUST be
`refs/store/<kind>/<entity-id>`.

A caller-selected name MUST be represented only as a compatibility alias and
MUST NOT be treated as the entity identity or as the source of truth. The
per-kind index is a materialized cache of canonical entity refs; entity refs
remain authoritative and a reader MUST be able to fall back when the index is
missing, malformed, or stale.
--

[role="requirement", id="plumbing.tombstone"]
.Typed Entity Deletion
--
`git store entity delete <kind> <entity-id>` MUST publish a typed tombstone
rather than prune the canonical ref. The tombstone MUST use the bound
`{schema/, value/}` frame and carry an explicit deleted state, kind, and
original `EntityId`. A repeated deletion MUST be distinguishable from an
absent entity.
--

[role="requirement", id="plumbing.migration-boundary"]
