...
A reader MUST carry the recognized generations as compiled-in constants; the genesis tree of this format version is `b82f18bc0b9f8c5d389d0ca161480365d72b08d6`.
...
`version` bound `Schema`'s own on-disk shape and the value codec a schema of that version describes together, in lockstep: a reader that rejected an unrecognized version thereby also refused to misread a value written by a newer codec, with no separate value-format version needed anywhere.
The `codec` entry above is what lets the schema-schema pin cover the same ground: `Schema`'s shape and the codec's spelling of every construct it can produce both live inside the generation's own tree, so either changing moves the generation id, exactly as an unrecognized `version` once would have refused both at once.
--
...
The genesis tree of this format version is `0afeeeb9f8e78c485199757eea274bb1d0e8a8db`.
